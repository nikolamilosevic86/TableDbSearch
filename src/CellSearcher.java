...
public class CellSearcher {
		...
		while (true) {
			...
			for (int i = start; i < end; i++) {
				...
				if (pmc_id != null) {
					...
					if(tableNo!=null)
					{
						System.out.println("	Table:"+doc.get("table_order"));
					}
					String title = doc.get("table_caption");
					if (title != null) {
						System.out.println("   table_caption: " + doc.get("table_caption"));
					}
					String cell_content = doc.get("cell_data");
					if (cell_content != null) {
						System.out.println("   cell_data: " + doc.get("cell_data"));
					}
					String cell_header = doc.get("cell_header");
					if (cell_header != null) {
						System.out.println("   cell_header: " + doc.get("cell_header"));
					}
					String cell_stub = doc.get("cell_stub");
					if (cell_stub != null) {
						System.out.println("   cell_stub: " + doc.get("cell_stub"));
					}
	...
	public static void PerformSearch(String queries, String index,
		...
		MultiFieldQueryParser parser = new MultiFieldQueryParser(
				Version.LUCENE_47, new String[] { "pmc_id", "pm_id",
						"table_order", "table_caption", "table_footer","article_title","spec_pragmatic","cell_header",
						"cell_stub","cell_superRow","cell_data"}, analyzer);
		while (true) {
			...
			CellSearcher.doPagingSearch(in, searcher, query, hitsPerPage, raw,
					true);
